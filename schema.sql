...
//new updates for school year 2026-2027 please paki add naman nito pre HAHAHAHAHA

(
    -- Grade 7 & 8 sections — inserts under the active school year
INSERT INTO sections (school_year_id, grade_level, name)
SELECT sy.id, 7, s.name
FROM (SELECT id FROM school_years WHERE is_active = 1 LIMIT 1) sy
JOIN (
    SELECT 'TUNGKUNG LANGIT' AS name UNION ALL SELECT 'VARANGAO' UNION ALL
    SELECT 'ALUNSINA' UNION ALL SELECT 'IDIANALE' UNION ALL SELECT 'AMIHAN' UNION ALL
    SELECT 'HALIYA' UNION ALL SELECT 'DUMAKULEM' UNION ALL SELECT 'KAPTAN' UNION ALL
    SELECT 'MAPULON' UNION ALL SELECT 'LAKAPATI' UNION ALL SELECT 'HANAN' UNION ALL
    SELECT 'AMAN SINAYA' UNION ALL SELECT 'INDARAPATRA' UNION ALL SELECT 'DUMANGAN' UNION ALL
    SELECT 'DALIKMATA' UNION ALL SELECT 'BANTUGAN' UNION ALL SELECT 'MAYARI' UNION ALL
    SELECT 'HUMADAPNON' UNION ALL SELECT 'URDUJA' UNION ALL SELECT 'MACANDUC' UNION ALL
    SELECT 'LIHANGIN' UNION ALL SELECT 'MAGWAYEN' UNION ALL SELECT 'SULAYMAN' UNION ALL
    SELECT 'LIADLAW' UNION ALL SELECT 'LAM-ANG' UNION ALL SELECT 'TUWAANG' UNION ALL
    SELECT 'APOLAKI' UNION ALL SELECT 'DIAN MASALANTA' UNION ALL SELECT 'LAKAMBINI' UNION ALL
    SELECT 'TALA' UNION ALL SELECT 'LIDAGAT' UNION ALL SELECT 'YNAGUINID' UNION ALL
    SELECT 'LALAHON'
) s;

INSERT INTO sections (school_year_id, grade_level, name)
SELECT sy.id, 8, s.name
FROM (SELECT id FROM school_years WHERE is_active = 1 LIMIT 1) sy
JOIN (
    SELECT 'ANDROMEDA' AS name UNION ALL SELECT 'AQUILA' UNION ALL SELECT 'ARA' UNION ALL
    SELECT 'AQUARIUS' UNION ALL SELECT 'ANTILA' UNION ALL SELECT 'AURIGA' UNION ALL
    SELECT 'CARINA' UNION ALL SELECT 'CASSIOPEIA' UNION ALL SELECT 'CENTAURUS' UNION ALL
    SELECT 'CEPHEUS' UNION ALL SELECT 'CHAMALEON' UNION ALL SELECT 'ARIES' UNION ALL
    SELECT 'DELPHINUS' UNION ALL SELECT 'DRACO' UNION ALL SELECT 'EQUULEUS' UNION ALL
    SELECT 'ERIDANUS' UNION ALL SELECT 'GEMINI' UNION ALL SELECT 'COLUMBA' UNION ALL
    SELECT 'HYDRA' UNION ALL SELECT 'ORION' UNION ALL SELECT 'LEO' UNION ALL
    SELECT 'LYNX' UNION ALL SELECT 'LYRA' UNION ALL SELECT 'PAVO' UNION ALL
    SELECT 'PEGASUS' UNION ALL SELECT 'TAURUS' UNION ALL SELECT 'URSA' UNION ALL
    SELECT 'VELA' UNION ALL SELECT 'VELORUM' UNION ALL SELECT 'VIRGO'
) s;
)

(
    -- 1) Rename the active school year to 2026-2027
UPDATE school_years
SET name = '2026-2027', is_active = 1
WHERE is_active = 1;

-- 2) Fix its three terms with the real DepEd dates
UPDATE terms t
JOIN school_years sy ON sy.id = t.school_year_id AND sy.name = '2026-2027'
SET t.start_date = '2026-06-08', t.end_date = '2026-09-04', t.name = 'Term 1'
WHERE t.term_no = 1;

UPDATE terms t
JOIN school_years sy ON sy.id = t.school_year_id AND sy.name = '2026-2027'
SET t.start_date = '2026-09-14', t.end_date = '2026-12-11', t.name = 'Term 2'
WHERE t.term_no = 2;
